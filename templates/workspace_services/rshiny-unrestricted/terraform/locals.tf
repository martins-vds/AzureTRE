locals {
  ...
  webapp_name                    = "rshiny-unrestricted-${local.service_resource_name_suffix}"
  core_resource_group_name       = "rg-${var.tre_id}"
  keyvault_name                  = lower("kv-${substr(local.workspace_resource_name_suffix, -20, -1)}")
  version                        = replace(replace(replace(data.local_file.version.content, "__version__ = \"", ""), "\"", ""), "\n", "")
  rshiny_image_repository        = "microsoft/azuretre/rshiny-workspace-service-unrestricted"
  ...
  rshiny_openid_auth = "${var.aad_authority_url}/${data.azurerm_key_vault_secret.aad_tenant_id.value}/v2.0"
}
